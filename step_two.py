...
def step_two(sims, aucs, nbs, briers, labels, texts, instances, samples_auc, samples_nb, samples_brier,
  ...
  matplotlib.rc('font', size=12) #controls default text size
  matplotlib.rc('axes', labelsize=15) #fontsize of the x and y labels
  matplotlib.rc('xtick', labelsize=14) #fontsize of the x tick labels
  matplotlib.rc('ytick', labelsize=14) #fontsize of the y tick labels
  ...
  fig.legend(scs, legend_labels, frameon=False, loc = 'lower center',  ncol=2)
  ...
  img.seek(0)
  plot2_data = urllib.parse.quote(base64.b64encode(img.read()).decode())
  return plot2_data
